...
demo=[by_id[x['id']] for x in raw['demo30']]; golden=[by_id[x['id']] for x in raw['golden50']]
...
def golden_table():
    ...
    for i,x in enumerate(golden,1):
        ...
        imgs=''.join(f'<img src="{fr}/{j}.jpg" onerror="this.remove()">' for j in (1,3,6))
        formula=t.get('formula','—').split(' + ')[:3]
        formula=' + '.join(formula)
        rows.append(f'<tr><td>{i}</td><td class="goldf">{esc(formula)}</td><td class="frames">{imgs}</td><td>{esc(t.get("audience","—"))}</td><td>{esc(t.get("cat","—"))}</td><td><a href="{esc(x["url"])}" target="_blank">视频</a></td></tr>')
    return f'<div class="scroll"><table class="golden"><thead><tr><th>排名</th><th>黄金公式（严格≤3个标签）</th><th>前15秒关键帧<br>开头 / 中间 / 结尾</th><th>目标人群</th><th>适合品类</th><th>素材url</th></tr></thead><tbody>{"".join(rows)}</tbody></table></div>'
