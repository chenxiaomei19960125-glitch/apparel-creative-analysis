...
SECTOR_VALUES = {
    'underwear': {
        '商品属性': [('适用季节','p0','春季 / 夏季凉感 / 秋季 / 冬季保暖 / 四季通用','厚薄、凉感/保暖面料和文案季节词。'),('功能属性','p0','无痕 / 舒适贴合 / 支撑聚拢 / 收腹提臀 / 透气抗菌 / 吸汗 / 保暖 / 防磨腿','贴身试穿、弹力/面料特写及字幕。'),('材质属性','p0','纯棉 / 莫代尔 / 冰丝 / 蕾丝 / 桑蚕丝 / 乳胶 / 锦纶氨纶','材质特写、吊牌或口播材质词。')],
...
def categories(rows):
    ...
    for name,rs in d.items():
        ...
        def top_tag(field):
            ...
            for r in rs: dist[r['tags'][field]] += r['spend']
            return max(dist,key=dist.get) if dist else '—'
        func=top_tag('function'); sell=top_tag('sell'); struct=top_tag('struct'); scene=top_tag('scene'); role=top_tag('role')
        why=f"以「{func}」回应类目核心决策，通过{sell}把结果可视化；{struct}搭配{scene}/{role}降低理解成本。"
        out.append({'name':name,'count':len(rs),'share':round(cost/total*100,1),'ctr':round(wavg(rs,'ctr'),2),'cvr':round(wavg(rs,'cvr'),2),'v3':round(wavg(rs,'v3'),1),'struct':struct,'scene':scene,'role':role,'function':func,'sell':sell,'why':why})
...
def home_html():
    ...
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>服饰行业创意分析</title>{STYLE}</head><body><main class="wrap"><header class="hero"><div class="eyebrow">AI 创意洞察 · 服饰行业专项</div><h1>服饰行业创意分析</h1><p>总站仅提供服饰行业整体标签树和四个赛道入口；各赛道进入后使用独立的「标签树结构 / 标签分析和结论 / 素材内容逐帧剖析」页面。</p></header><div class="home-tabs"><button class="home-tab active" data-view="aigc">🤖 AIGC 使用情况</button><button class="home-tab" data-view="overview">📊 行业概览</button></div><div class="home-view active" id="view-aigc">{aigc_view}</div><div class="home-view" id="view-overview"><section class="section"><h2>进入赛道分析</h2><p>箱包鞋靴保留已上线的原站内容；内衣、男装和女装分别进入独立分析站。</p><div class="entry-grid">{entries}</div></section><section class="section"><h2>服饰行业整体标签树结构</h2><p>内容形式、画面客观要素为最优先的创意标签；其余标签用于补足商品决策、卖点表达和情绪营销解释。</p><div class="tree-wrap">{tree_html(OVERALL_TREE, overall=True)}</div></section></div><footer class="footer">服饰创意 XAI · 统一标签语言、效果归因与素材复用</footer></main><script>document.querySelectorAll('.home-tab').forEach(b=>b.onclick=()=>{{document.querySelectorAll('.home-tab').forEach(x=>x.classList.toggle('active',x===b));document.querySelectorAll('.home-view').forEach(x=>x.classList.toggle('active',x.id==='view-'+b.dataset.view))}})</script></body></html>'''
...
def sector_html(payload):
    ...
    for key,title in dim_names.items():
        ...
        dims.append(f'<article class="dim {"core" if key in {"struct","role","scene"} else ""}"><div class="dim-head"><h3>{title}</h3><span class="scope">大盘</span></div><div class="scroll"><table class="table"><thead><tr><th>标签</th><th>消耗占比</th><th>CTR</th><th>3秒完播</th><th>CVR</th></tr></thead><tbody>{rows}</tbody></table></div></article>')
    ...
    cats=''.join(f'<tr><td>{i+1}</td><td class="name">{esc(x["name"])}</td><td>{x["count"]}</td><td class="hi">{x["share"]}%</td><td class="hi">{x["ctr"]}%</td><td>{x["v3"]}%</td><td>{x["cvr"]}%</td><td><b>制作：</b>{esc(x["struct"])}<br><b>场景/角色：</b>{esc(x["scene"])} / {esc(x["role"])}<br><b>卖点：</b>{esc(x["function"])} · {esc(x["sell"])}</td><td class="why">{esc(x["why"])}</td></tr>' for i,x in enumerate(payload['categories']))
    ...
    return f"""<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{payload['name']}创意分析</title>{SECTOR_STYLE}</head><body><main class="wrap"><a class="back" href="../">← 返回服饰行业总站</a><header class="hero"><div>AI 创意洞察 · {payload['name']}赛道</div><h1>{payload['emoji']} {payload['name']}创意分析</h1><p>{payload['desc']} · 标签树、效果分析与 Top 素材逐帧创意拆解。</p></header><div class="tabs"><button class="tab active" data-tab="tree">🌳 素材标签树结构</button><button class="tab" data-tab="analysis">🏷 标签分析和结论</button><button class="tab" data-tab="materials">🎬 素材内容 · 逐帧剖析</button></div><div class="view active" id="tree"><section class="section"><h2>{payload['name']}赛道标签树结构</h2><p>内容形式、画面客观要素使用统一核心标签；商品属性、卖点表达、情绪营销已按 {payload['name']} 的实际类目与素材特点调整枚举值。</p><div class="tree-wrap">{tree}</div></section></div><div class="view" id="analysis"><section class="section"><h2>大盘标签分析 · 一分钟看懂本期</h2><div class="kpis"><div class="kpi"><b>{payload['metrics']['materials']}</b><span>有效去重素材</span></div><div class="kpi"><b>{payload['metrics']['ctr']}%</b><span>消耗加权 CTR</span></div><div class="kpi"><b>{payload['metrics']['v3']}%</b><span>消耗加权 3秒完播</span></div><div class="kpi"><b>{payload['metrics']['cvr']}%</b><span>消耗加权 浅层CVR</span></div></div><div class="insight">{esc(payload['narrative'])}</div></section><section class="section"><h2>各标签维度明细</h2><p>严格按箱包鞋靴分析页的逻辑展示：制作形式优先，其次是画面客观要素，再下钻商品属性、卖点表达和情绪营销。</p>{analysis_blocks}</section><section class="section category-section"><h2>不同品类的主流素材打法（消耗 Top10 类目）</h2><p>按箱包鞋靴页同一口径：展示数量/消耗效果、主流标签组合以及为什么这类标签跑得好。</p><div class="scroll"><table class="table category-table"><thead><tr><th>#</th><th>细分类目</th><th>素材数</th><th>消耗占比</th><th>CTR</th><th>3秒完播</th><th>CVR</th><th>主流标签组合</th><th>为什么这类标签跑得好</th></tr></thead><tbody>{cats}</tbody></table></div></section></div><div class="view" id="materials"><section class="section"><h2>Top 跑量素材逐帧剖析（最多 12 条）</h2><p>按消耗排序；每条均展示视频、6 个关键帧、叙事路径、Learning 与创意标签，布局与箱包鞋靴深度站保持一致。</p><div class="material-list">{''.join(materials)}</div><div class="note">{esc(payload['note'])}</div></section></div></main><script>document.querySelectorAll('.tab').forEach(b=>b.onclick=()=>{{document.querySelectorAll('.tab').forEach(x=>x.classList.toggle('active',x===b));document.querySelectorAll('.view').forEach(x=>x.classList.toggle('active',x.id===b.dataset.tab))}})</script></body></html>"""
