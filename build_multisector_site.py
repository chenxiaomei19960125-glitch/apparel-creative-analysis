...
OVERALL_TREE = [
    ...
    ('内容形式', '最核心：先判断素材如何被拍成一个可消费的视频。', True, [
        ('视频结构类型', 'p00', '真人口播 / 多人剧情 / 沉浸式产品展示 / 直播切片 / AI数字人口播', '有无真人对镜头说话；是否多人互动/剧情冲突；是否仅产品/手部；是否有直播浮层或AI合成痕迹。', '素材创意体裁的第一切分维度。', '素材创意形式，效果归因第一切分维度。', '开屏 3 秒决定是否进入观看与后续转化。'),
    ]),
...
def tree_html(tree, overall=False):
    ...
    for title,desc,core,children in tree:
        if overall:
            ...
            body=f'<div class="tree-scroll"><table class="overall-table"><thead><tr><th>二级标签</th><th>优先级</th><th>Value/枚举值</th><th>判断依据 / 画面特征</th><th>一级标签分类说明</th><th>二级标签分类说明</th><th>开屏/种草关键</th></tr></thead><tbody>{cards}</tbody></table></div>'
        else:
...
def sector_html(payload):
    tree = tree_html(sector_tree(payload['id']))
    ...
    for i,x in enumerate(payload['top']):
        ...
        groups=[('视频结构类型',t['struct'],True),('出镜角色',t['role'],True),('拍摄场景',t['scene'],True),('产品卖点',t['sell'],False),('场景人群',t['people'],False),('产品功效',t['function'],False),('用户痛点',t['pain'],False),('情绪钩子',t['emotion'],False)]
